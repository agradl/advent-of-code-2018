...
function max(histogram, selector){
    ...
    Object.keys(histogram).forEach(key => {
        ...
        if (val > max.value){
            max.value = val
            max.item = { key: key, value: histogram[key], selectorValue: val };
        }
    });
    return max.item;
}

var guardSummaryWithHighestMinuteFrequency = max(guardMinutes, (obj) => {
    return max(obj.histogram, val => val).value;
});

var minute = max(guardSummaryWithHighestMinuteFrequency.value.histogram, val => val).key


console.log('the answer is ' + (Number.parseInt(guardSummaryWithHighestMinuteFrequency.key,10) * minute));
